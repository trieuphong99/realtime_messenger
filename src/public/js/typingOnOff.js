...
$(document).ready(function () {
  // listen typing On
  socket.on("user-is-typing-response", function (response) {
  ...
  // listen tying Off
  socket.on("user-is-not-typing-response", function (response) {
    if (response.currentGroupId) {
      if (response.currentUserId !== $("#dropdown-navbar-user").data("uid")) {
        $(`.chat[data-chat=${response.currentGroupId}]`)
          .find("div.bubble-typing-gif")
          .remove();
        nineScrollRight(response.currentGroupId);
      }
    } else {
      $(`.chat[data-chat=${response.currentUserId}]`)
        .find("div.bubble-typing-gif")
        .remove();
      nineScrollRight(response.currentUserId);
    }
